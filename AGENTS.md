...
- Use `mvn` directly; do not use the Maven Wrapper `mvnw`. This is because our build environment
  requires a specific, pre-installed Maven version for compatibility with our CI/CD pipeline, and
  using the wrapper could lead to version mismatches.
...
- Java: use JDK 25 (see `pom.xml` property `java.version`).
